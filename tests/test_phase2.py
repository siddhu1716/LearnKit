...
def test_evaluator_bounds_and_repair():
    ...
    result = evaluator.evaluate_with_llm_judge(
        ...
        reasoning_trace="Reasoning info",
        lm=lm
    )
    ...
    result_sq = evaluator.evaluate_with_llm_judge("task", "response", lm=lm_single_quotes)
    assert result_sq.score == 3.8
    assert result_sq.metadata["parse_method"] == "json_replace_quotes"
    
    # 3. JSON Repair - Regex Recovery
    lm_malformed = MockLM("Some prefix text { \"score\": 4.2, \"reasoning\": \"Indeed corrected\" } some suffix text")
    result_malformed = evaluator.evaluate_with_llm_judge("task", "response", lm=lm_malformed)
